...
void ViewController::launch(FileData* game, LaunchGameOptions options, Vector3f center, bool allowCheckLaunchOptions)
{
	...
	bool customEntrySplash = false;
	if (!Settings::getInstance()->getBool("HideWindow"))
		customEntrySplash = mWindow->playGameEntrySplash(game->getSourceFileData()->getSystem()->getName());

	if (!customEntrySplash && !Settings::getInstance()->getBool("HideWindow"))
		mWindow->setGameSplashScreen(game->getSourceFileData()->getSystem()->getName(), game->getImagePath(), game->getName(), game);
...
bool ViewController::input(InputConfig* config, Input input)
{
	...
	// TurboRama kiosk: botao Start = menu principal com senha admin
	if(config->isMappedTo("start", input) && input.value != 0)
	{
		GuiMenu::requestMainMenuAccess_static(mWindow);
		return true;
	}
...
void ViewController::update(int deltaTime)
{
	...
	mCreditHudElapsedMs -= deltaTime;
	if (mCreditHudElapsedMs <= 0)
	{
		mCreditHudElapsedMs = 500;
		if (CreditManager::getInstance().isEnabled() && CreditManager::getInstance().isShowHud())
			mCreditHudText = CreditManager::getInstance().formatHudLine();
		else
		{
...
void ViewController::render(const Transform4x4f& parentTrans)
{
	...
	const bool onHomeScreen =
		(mState.viewing == SYSTEM_SELECT || mState.viewing == START_SCREEN)
		&& (mWindow->peekGui() == this);

	if (onHomeScreen
		&& CreditManager::getInstance().isEnabled()
		&& CreditManager::getInstance().isShowHud()
		&& !mCreditHudText.empty())
	{
		// Menor que FONT_SIZE_MINI (~0.030) → ~0.022 da menor dimensao
		const unsigned int hudFontPx = (unsigned int)(0.022f * Math::min(
			(int)Renderer::getScreenHeight(), (int)Renderer::getScreenWidth()));
		auto font = Font::get(std::max(10u, hudFontPx));
		if (font)
		{
			auto& cm = CreditManager::getInstance();
			// Texto (nome / Avulso) em verde
			std::string namePart;
			const std::string player = cm.getCurrentPlayerName();
			if (!player.empty())
				namePart = player + " ";
			else if (cm.getRemainingSeconds() > 0)
				namePart = "Avulso ";
			else
				namePart = "— ";

			// Numeracao (tempo) em branco; || se pausado
			std::string timePart = cm.formatRemaining();
			if (cm.isSessionPaused())
				timePart = std::string("|| ") + timePart;

			const float th = std::max(font->sizeText(namePart).y(), font->sizeText(timePart).y());
			const float y = Renderer::getScreenHeight() * 0.9965f - th
				- Renderer::getScreenHeight() * 0.065f;
			const float x = Renderer::getScreenWidth() * 0.045f;

			Renderer::setMatrix(parentTrans);

			const unsigned int colName = cm.isCounting() ? 0x33FF66FF : 0x66CC66FF; // verde
			const unsigned int colTime = 0xFFFFFFFF; // branco

			// sombra
			if (!namePart.empty())
			{
				std::unique_ptr<TextCache> sh(
					font->buildTextCache(namePart, x + 1.f, y + 1.f, 0x000000AA));
				if (sh) font->renderTextCache(sh.get());
			}
			{
				const float xTime = x + font->sizeText(namePart).x();
				std::unique_ptr<TextCache> sh(
					font->buildTextCache(timePart, xTime + 1.f, y + 1.f, 0x000000AA));
				if (sh) font->renderTextCache(sh.get());
			}

			// nome verde
			std::unique_ptr<TextCache> drawName(
				font->buildTextCache(namePart, x, y, colName));
			if (drawName)
				font->renderTextCache(drawName.get());

			// numeros brancos
			const float xTime = x + font->sizeText(namePart).x();
			std::unique_ptr<TextCache> drawTime(
				font->buildTextCache(timePart, xTime, y, colTime));
			if (drawTime)
...
std::vector<HelpPrompt> ViewController::getHelpPrompts()
{
	...
	// TurboRama: prompt MENU/Start removido (menu desativado no kiosk)
	return prompts;
}
