// Adapted upstream licensing verifier; see UPSTREAM.md. Synthetic fixtures only.
using System;
...
public static class SuiteProtocolVerifier
{
    ...
    private static void VerifyNonForgeableCapabilityAndDefaultFailClosed()
    {
        ...
        True(typeof(SuiteLicensingRuntime).GetMethod("ActivateAndOpenAsync") is null,
            "EmulationStation must not expose activation");
        True(typeof(SuiteLicenseClient).GetMethod("ActivateAsync") is null,
            "EmulationStation must not call activation endpoints");
        var runtime = new SuiteLicensingRuntime("AUTHORITY_CONFIGURATION_MISSING", TimeProvider.System);
        try
        {
