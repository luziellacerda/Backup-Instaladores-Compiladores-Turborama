...
namespace Utils
{
	namespace Zip
	{
		...
		bool ZipFile::loadFromMemory(const void* data, size_t size)
		{
			try
			{
				if (mZipFile != nullptr)
				...
				mMemoryBuffer.assign((const unsigned char*)data, (const unsigned char*)data + size);

				mz_zip_archive* zipArchive = new mz_zip_archive();
				memset(zipArchive, 0, sizeof(mz_zip_archive));
				mz_bool status = mz_zip_reader_init_mem(zipArchive, mMemoryBuffer.data(), mMemoryBuffer.size(), 0);
				if (!status)
				{
					delete zipArchive;
					mMemoryBuffer.clear();
			...
			{
				LOG(LogError) << "ZipFile::loadFromMemory error : " << e.what();
			}
			catch (...)
			{
				LOG(LogError) << "ZipFile::loadFromMemory unknown error";
			}
