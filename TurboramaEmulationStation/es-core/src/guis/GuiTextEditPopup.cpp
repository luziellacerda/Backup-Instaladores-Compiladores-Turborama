...
GuiTextEditPopup::GuiTextEditPopup(Window* window, const std::string& title, const std::string& initValue, 
	const std::function<void(const std::string&)>& okCallback, bool multiLine, const char* acceptBtnText, bool passwordMode)
	: GuiComponent(window), mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 3)), mMultiLine(multiLine)
{
	...
	// Em senha: CLEAR limpa o campo (nao submete string vazia como RESET)
	if (passwordMode)
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, _("CLEAR"), _("CLEAR"), [this] { mText->setValue(""); mText->setCursor(0); }));
	else
