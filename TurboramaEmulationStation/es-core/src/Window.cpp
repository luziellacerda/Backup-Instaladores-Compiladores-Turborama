...
void Window::update(int deltaTime)
{
	...
	if (mFrameTimeElapsed > 250)
	{
		...
		if (Settings::DrawFramerate())
		{
			...
			size_t max_ram = Settings::getInstance()->getInt("MaxRAM");

			int queueSize = TextureResource::getQueueSize();

			ss << "\nFont VRAM: " << fontVramUsageMb << " Tex VRAM: " << textureVramUsageMb << " Cached Tex RAM: " << textureCacheUsageMb << " Known Tex: " << textureKnownUsageMb << " Max VRAM: " << max_texture << " Max RAM: " << max_ram << " Queued : " << queueSize;
...
void Window::setGameSplashScreen(const std::string& systemName, std::string fallbackImage, std::string customText, IBindable* bindable)
{
	if (Settings::getInstance()->getBool("HideWindow"))
		return;

	auto media = GameSplash::resolve(systemName, GameSplash::Kind::ENTRY);
	if (media.valid())
	{
		mSplash = std::make_shared<Splash>(this, media);
		mSplash->startPlayback();
	}
	else if (!Utils::FileSystem::exists(fallbackImage))
		mSplash = std::make_shared<Splash>(this, getCustomSplashScreenImage(), false, bindable);
	else
		mSplash = std::make_shared<Splash>(this, fallbackImage, false, bindable);

	mSplash->update(customText);
}

void Window::playBlockingGameSplash(const GameSplash::Media& media)
{
	closeSplashScreen();
	mSplash = std::make_shared<Splash>(this, media);
	mSplash->startPlayback();

	while (!mSplash->isPlaybackFinished())
	{
		mSplash->tick(16);
		mSplash->render(1.0f, true);
		SDL_Delay(16);
...
bool Window::playGameEntrySplash(const std::string& systemName)
{
	if (Settings::getInstance()->getBool("HideWindow"))
		return false;
	...
	playBlockingGameSplash(media);
	return true;
}

void Window::displayGameExitSplash(const std::string& systemName)
{
	if (Settings::getInstance()->getBool("HideWindow"))
		return;

	auto media = GameSplash::resolve(systemName, GameSplash::Kind::EXIT);
	if (!media.valid())
		return;
