...
std::string ThemeData::getCurrentThemeRootPath()
{
	...
	if (EmbeddedTheme::isActiveThemeSet(selectedTheme))
		return EmbeddedTheme::getRootPath();

	auto themeSets = ThemeData::getThemeSets();
	if (themeSets.empty())
		return "";

	std::map<std::string, ThemeSet>::const_iterator set = themeSets.find(selectedTheme);
...
std::string ThemeData::getThemeFromCurrentSet(const std::string& system)
{
	...
	if (EmbeddedTheme::isAvailable() && (selectedTheme.empty() || selectedTheme == EmbeddedTheme::THEME_SET_ID || selectedTheme == "default" || Utils::String::toLower(selectedTheme) == "turborama"))
	{
		Settings::getInstance()->setString("ThemeSet", EmbeddedTheme::THEME_SET_ID);
		return EmbeddedTheme::getThemePath(system);
	}
