...
TextEditComponent::TextEditComponent(Window* window) : GuiComponent(window),
	...
	mScrollOffset(0.0f, 0.0f), mCursor(0), mEditing(false), mPasswordMode(false),
...
void TextEditComponent::setPasswordMode(bool enabled)
{
...
{
	// Em modo senha: mostra asteriscos, mantem mText real
	std::string display = mText;
	if (mPasswordMode && !mText.empty())
	{
		size_t n = 0;
		for (size_t i = 0; i < mText.size(); )
		{
			i = Utils::String::nextCursor(mText, i);
			n++;
		}
		display.assign(n, '*');
	}

	std::string wrappedText = (isMultiline() ? mFont->wrapText(display, getTextAreaSize().x()) : display);
...
void TextEditComponent::onCursorChanged()
{
	if(isMultiline())
	{
		...
		// Cursor visual: em senha usa asteriscos ate a posicao
		std::string before = mText.substr(0, mCursor);
		if (mPasswordMode && !before.empty())
		{
			size_t n = 0;
			for (size_t i = 0; i < before.size(); )
			{
				i = Utils::String::nextCursor(before, i);
				n++;
			}
			before.assign(n, '*');
		}
		Vector2f cursorPos = mFont->sizeText(before);
