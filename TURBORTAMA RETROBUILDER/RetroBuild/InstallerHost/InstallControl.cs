...
this.wizardHeader.Text = Texts.GetString("InstallTitle", Array.Empty<object>());
		...
		{
			FileStream fileStream = new FileStream(Application.ExecutablePath, FileMode.Open, FileAccess.Read, FileShare.Read);
			...
			if (num <= 0L || num2 < 0L || num2 >= fileStream.Length)
			{
				fileStream.Dispose();
						...
						using (Stream inputStream = zipFile.GetInputStream(zipEntry))
						{
							using (FileStream fileStream = File.Create(text))
							{
								...
								while ((num4 = inputStream.Read(array, 0, array.Length)) > 0)
								{
									...
									if (num > 0L)
									{
										int num5 = (int)Math.Max(0L, Math.Min(100L, num2 * 100L / num));
										if (num5 != num3)
										{
