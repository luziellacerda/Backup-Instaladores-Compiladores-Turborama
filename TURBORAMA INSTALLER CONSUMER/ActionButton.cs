...
namespace TurboRama.Next
{
    ...
    internal class ActionButton : Button
    {
        ...
        public ActionButton()
        {
            ...
            UseVisualStyleBackColor = false; BackColor = Palette.Background;
        ...
        // ButtonBase does not reliably erase a transparent owner-painted button.
        // Every dirty region must be initialized, including the rounded corners
        // and quiet states. Never copy another control's buffer or parent paint.
        protected override void OnPaintBackground(PaintEventArgs e)
        { PaintSurfaceBackground(e); }
        private void PaintSurfaceBackground(PaintEventArgs e)
        {
            Color background = SystemInformation.HighContrast ? SystemColors.Control : Palette.Background;
            if (!SystemInformation.HighContrast)
                for (Control ancestor = Parent; ancestor != null; ancestor = ancestor.Parent)
                    if (ancestor.BackColor.A == 255) { background = ancestor.BackColor; break; }
            using (Brush brush = new SolidBrush(background))
                e.Graphics.FillRectangle(brush, Rectangle.Intersect(ClientRectangle, e.ClipRectangle));
        }
        ...
        private void DrawContent(Graphics g, Rectangle body, Color ink, float dpi)
        {
            ...
            TextFormatFlags flags = TextFormatFlags.VerticalCenter | TextFormatFlags.SingleLine | TextFormatFlags.EndEllipsis | TextFormatFlags.NoPrefix | TextFormatFlags.PreserveGraphicsClipping;
        ...
        private void DrawCompound(Graphics g, Rectangle body, Color ink, float dpi)
        {
            ...
            TextRenderer.DrawText(g, Text, Font, new Rectangle(text.X, text.Y, text.Width, titleHeight), ink,
                TextFormatFlags.SingleLine | TextFormatFlags.EndEllipsis | TextFormatFlags.NoPrefix | TextFormatFlags.PreserveGraphicsClipping);
            using (Font detailFont = new Font(Font.FontFamily, Math.Max(9f, Font.Size - 1), FontStyle.Regular))
                TextRenderer.DrawText(g, Description, detailFont, new Rectangle(text.X, text.Y + titleHeight + Px(6, dpi), text.Width, Math.Max(1, text.Height - titleHeight)),
                    highContrast ? ink : Enabled ? Palette.Muted : ink, TextFormatFlags.WordBreak | TextFormatFlags.EndEllipsis | TextFormatFlags.NoPrefix | TextFormatFlags.PreserveGraphicsClipping);
