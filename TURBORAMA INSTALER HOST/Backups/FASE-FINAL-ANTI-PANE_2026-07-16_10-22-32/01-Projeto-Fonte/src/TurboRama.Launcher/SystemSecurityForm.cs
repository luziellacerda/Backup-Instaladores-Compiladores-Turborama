...
internal sealed class SystemSecurityForm : Form
{
    ...
    public SystemSecurityForm(string pin)
    {
        ...
        _status = new Label
        {
            Text = "Senha = conta Arcade. Esc cancela.",
            Left = pad,
            Top = y,
            Width = innerW,
            Height = 22,
    ...
    private void OnClick(SecurityAction action, bool needPin)
    {
        ...
        if (needPin)
        {
            ...
            // Trim evita espaços acidentais; senha de fábrica = Lz2026@$
            string typed = (_pinBox.Text ?? "").Trim();
            string expected = (_pin ?? "").Trim();
            if (!string.Equals(typed, expected, StringComparison.Ordinal))
            {
                _status.ForeColor = Red;
                _status.Text = "Senha incorreta. Use a senha kiosk (mesma do Arcade).";
                _pinBox.Clear();
                _pinBox.Focus();
                return;
            }
    ...
    public static string ResolvePin(ProductConfiguration config) =>
        FactoryDefaults.ResolveSecurityMenuPin(config);
