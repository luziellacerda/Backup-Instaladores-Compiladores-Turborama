    ...
    private static string RegisterSecurityAgent()
    {
        try
        {
            ...
            try
            {
                ...
                Task("/Delete /TN \"TurboRamaSecurityAgent\" /F");
                Task("/Create /TN \"TurboRamaSecurityAgent\" /SC ONLOGON /RL LIMITED /F " +
                     "/TR \"\\\"" + launcher + "\\\" --security-agent\"");
                // Keep-alive a cada 2 min (mutex evita duplicar)
                Task("/Delete /TN \"TurboRamaSecurityAgentKeepAlive\" /F");
                Task("/Create /TN \"TurboRamaSecurityAgentKeepAlive\" /SC MINUTE /MO 2 /RL LIMITED /F " +
                     "/TR \"\\\"" + launcher + "\\\" --security-agent\"");
            }
            catch
            {
            ...
            return "SecurityAgent Ctrl+End + keep-alive 2min";
        }
        catch (Exception ex)
        {
