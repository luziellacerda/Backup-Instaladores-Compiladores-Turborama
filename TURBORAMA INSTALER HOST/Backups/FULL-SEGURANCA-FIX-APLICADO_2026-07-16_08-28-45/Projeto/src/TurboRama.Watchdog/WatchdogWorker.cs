...
public sealed class WatchdogWorker
{
    ...
    public async Task RunAsync(CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                ...
                // Garantir agent Ctrl+End (heartbeat) sempre que haja sessão interativa
                if (hasLauncher || InteractiveSessionLauncher.HasInteractiveConsoleSession())
                {
                    EnsureSecurityAgent(Path.Combine(ProductPaths.AppLauncher, "TurboRama.Launcher.exe"));
                }
            }
    ...
    /// <summary>
    /// Se security-agent-alive.txt estiver velho (&gt;45s), arranca --security-agent.
    /// O agent usa Mutex — não duplica se já estiver vivo.
    /// </summary>
    private void EnsureSecurityAgent(string launcherExe)
    {
        try
        {
            if (MaintenanceLock.IsActive())
            ...
            {
                fresh = DateTime.Now - File.GetLastWriteTime(alive) < TimeSpan.FromSeconds(45);
            }
            ...
            _logger.Warning("Watchdog", "Security agent heartbeat stale — a recuperar Ctrl+End…");

            if (InteractiveSessionLauncher.TryStartInActiveSession(
                    launcherExe,
                    ProductPaths.AppLauncher,
                    out string detail,
                    arguments: "--security-agent"))
            {
                _logger.Info("Watchdog", "Security agent pedido na sessão: " + detail);
            }
            else
            {
                // fallback Process.Start (pode falhar em Session 0 — já filtrado)
                try
                {
                    ...
                    _logger.Info("Watchdog", "Security agent Process.Start fallback.");
                }
                catch (Exception ex)
                {
                    _logger.Error("Watchdog", "Security agent recover fail: " + ex.Message, errorCode: "WD_SEC");
                }
            }
        }
        catch (Exception ex)
        {
            _logger.Warning("Watchdog", "EnsureSecurityAgent: " + ex.Message);
        }
    }
}
