...
internal static class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        // Exceto --test-loading (já tratado acima)
        if (IsAdministratorUser(user) || IsMachineOrServiceAccount(user))
        {
            ...
            if (Environment.UserInteractive && !IsSessionZero())
            {
                TryShowInfoOnce(
                    ...
                    "Para ver a loading sem reiniciar:\n" +
                    "  TurboRama.Launcher.exe --test-loading\n" +
                    "ou TESTAR-LOADING.bat");
            }
        ...
        while (true)
        {
            ...
            try
            {
                ...
                if (!bootLoadingDone && config.ShowLoadingScreen)
                {
                    ...
                    // 1) Mostrar a tela PRIMEIRO (desktop/áudio do shell estabilizam)
                    // 2) Depois tocar boot.wav (PlaySound + retries — evita falha silenciosa no logon)
                    ...
                    // Hold visual + som (callback leve — sem sleeps longos)
                    ...
                    // SEMPRE fechar loading antes de abrir o jogo (nunca ficar em 100%)
                    try
                    {
                        loading.HideLoading();
                    }
                    ...
                    try
                    {
                        ...
                        // Menu Desligar (ou Sair do sistema) no TurboRama: NÃO reiniciar o jogo —
                        // mostrar tela de desligar e desligar o Windows.
                        RunPowerOffAfterFrontendExit();
                        break;
                    }
                    finally
                    {
                        proc?.Dispose();
                        // reforço: garantir que a form não ficou no ecrã
                ...
                else
                {
                    ...
                    // Menu Desligar no TurboRama → splash + power off Windows
                    RunPowerOffAfterFrontendExit();
                    break;
                }
            }
    ...
    /// <summary>
    /// TurboRama/ES fechou (opção Desligar no menu): splash TurboRama + desligar Windows.
    /// Não mexe na loading intro.
    /// </summary>
    private static void RunPowerOffAfterFrontendExit()
    {
        _logger?.Info("Launcher",
            "Frontend encerrado — a mostrar tela de desligar e a desligar o Windows.");

        bool shutOk = false;
        string shutMsg = string.Empty;
        try
        {
            // ~2.8s animação cinemática → power off → mantém splash ~10s por cima do Windows
            ShutdownScreenForm.ShowAndHold(
                holdMsBefore: 2800,
                holdMsAfter: 10000,
                shutdownAction: () =>
                {
                    shutOk = PowerShutdownHelper.ShutdownNow(out shutMsg, _logger);
                    _logger?.Info("Launcher", "Shutdown resultado ok=" + shutOk + " " + shutMsg);
                });
        }
        catch (Exception ex)
        {
        ...
        if (!shutOk)
        {
            _logger?.Error("Launcher", "Falha ao desligar PC: " + shutMsg, errorCode: "TR-SHUT");
            if (Environment.UserInteractive)
            {
                try
                {
                    MessageBox.Show(
                        "Não foi possível desligar o PC.\n" + shutMsg,
                        "TurboRama",
