        ...
        while (true)
        {
            ...
            try
            {
                ...
                if (!bootLoadingDone && config.ShowLoadingScreen)
                {
                    ...
                    // 1) Som + marca no ecrã (sem TurboRama.exe ainda)
                    bool soundOk = sound.TryPlay(config.LoadingSoundFile, _logger);
                    _logger.Info("Launcher",
                        "BootSound ok=" + soundOk + " path=" + BootSoundPlayer.ResolveSoundPath(config.LoadingSoundFile));

                    loading.ShowBrandHold(brandMs, (p, st) =>
                    {
                        // só pump — status já na form
                    });

                    _logger.Info("Launcher", "Marca TURBORAMA concluída — a iniciar frontend.");

                    // 2) Agora sim o jogo (loading ainda TopMost um instante, depois esconde)
                    using Process? proc = Process.Start(psi);
                    int pid = proc?.Id ?? 0;
                    _logger.Info("Launcher", "Frontend iniciado PID=" + (pid > 0 ? pid.ToString() : "?"));

                    loading.SetStatus("A abrir o arcade...");
                    ...
                    _logger.Info("Launcher", "Loading de logon encerrado (marca + arranque).");
