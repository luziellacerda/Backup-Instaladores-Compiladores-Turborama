...
internal static class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        // Menu segurança TurboRama (Ctrl+End) — agente autónomo + keep-alive
        ...
        // Alerta disco baixo (não bloqueia kiosk)
        try
        {
            var drive = new DriveInfo(Path.GetPathRoot(ProductPaths.Root) ?? "C:\\");
            if (drive.IsReady && drive.AvailableFreeSpace < 10L * 1024 * 1024 * 1024)
            {
                _logger.Warning("Launcher",
                    "Disco com pouco espaço livre: " +
                    (drive.AvailableFreeSpace / (1024 * 1024)) + " MB em " + drive.Name);
            }
        }
        catch (Exception ex)
        {
            _logger.Warning("Launcher", "Disk check: " + ex.Message);
        }
        ...
        var lastAgentCheck = DateTime.MinValue;
        while (true)
        {
            // Recuperar agent Ctrl+End se heartbeat morreu (a cada ~30s no loop)
            if (config.EnableSecurityMenu &&
                (DateTime.UtcNow - lastAgentCheck).TotalSeconds >= 30)
            {
                lastAgentCheck = DateTime.UtcNow;
                try { SecurityAgentHost.EnsureRunning(_logger); }
                catch (Exception ex) { _logger.Warning("Launcher", "EnsureAgent: " + ex.Message); }
            }
    ...
    {
        // Aceita pasta D:\Turborama\... e layout flat D:\TurboRama.exe (setup estável / cópia modelo)
        return FactoryDefaults.ResolveFrontendExecutable(
            configuredPath: config.FrontendExecutable,
            packFrontendDir: ProductPaths.Frontend);
    }
