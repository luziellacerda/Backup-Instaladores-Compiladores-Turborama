...
public sealed class PostInstallValidationService
{
    public PreflightReport Run(ProductConfiguration config, bool clearLocks = false)
    {
        ...
        // Frontend — pasta clássica OU layout flat (D:\TurboRama.exe). Nunca é erro duro de fábrica.
        string? foundFe = FactoryDefaults.FindExistingFrontend(
        ...
        if (foundFe is not null)
        {
            ...
            // Se o JSON aponta path morto, avisa para rebind (seed/Launcher já resolvem no boot)
            if (!string.IsNullOrWhiteSpace(config.FrontendExecutable) &&
                !string.Equals(config.FrontendExecutable, foundFe, StringComparison.OrdinalIgnoreCase) &&
                !File.Exists(config.FrontendExecutable))
            {
                report.AddWarning(
                    "Config frontend apontava para path inexistente (" + config.FrontendExecutable +
                    "); EXE real: " + foundFe + ". Seed/Launcher usam o path real.",
                    "V_FRONTEND_REBIND");
            }
        }
        else
        {
            report.AddWarning(
                "Frontend não encontrado (kiosk sobe; copie jogos para D:\\TurboRama.exe ou D:\\Turborama\\TurboRama.exe).",
                "V_FRONTEND");
        }
