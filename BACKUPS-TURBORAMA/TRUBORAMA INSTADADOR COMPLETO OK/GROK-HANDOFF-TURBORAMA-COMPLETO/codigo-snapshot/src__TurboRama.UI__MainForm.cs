...
internal sealed class MainForm : Form
{
    ...
    public MainForm(ProductConfiguration config, ITurboRamaLogger logger)
    {
        ...
        // Cabe em monitores pequenos / 1366x768 / kiosk 720p (evita botões fora da tela)
        int screenW = Screen.PrimaryScreen?.WorkingArea.Width ?? 1280;
        int screenH = Screen.PrimaryScreen?.WorkingArea.Height ?? 720;
        Width = Math.Min(940, Math.Max(720, screenW - 40));
        Height = Math.Min(760, Math.Max(520, screenH - 40));
        MinimumSize = new Size(640, 480);
        MaximumSize = new Size(screenW, screenH);
        AutoScroll = true;
