...
public sealed class PreflightService
{
    public PreflightReport Run(ProductConfiguration config)
    {
        ...
        // Frontend (aviso apenas — jogos podem ser copiados depois do kiosk)
        {
            ...
            else if (!string.IsNullOrWhiteSpace(config.FrontendExecutable))
            {
                report.AddWarning(
                    "Frontend ainda não no disco (OK se copiar depois): " + config.FrontendExecutable +
                    " — também aceita D:\\TurboRama.exe (layout flat).",
                    "PF_FRONTEND");
            }
        }
