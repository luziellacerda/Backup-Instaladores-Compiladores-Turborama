...
internal static class Program
{
    ...
    private static bool _operatorUiBusy;
    private static SecureDesktopWatcher? _secureDesktop;
    ...
    private static void Main(string[] args)
    {
        ...
        // Preview do painel segurança (Ctrl+End) sem kiosk
        if (HasArg(a, "--test-operator") || HasArg(a, "--preview-operator") || HasArg(a, "/test-operator") ||
            HasArg(a, "--test-security-ui") || HasArg(a, "--preview-security"))
        {
            RunOperatorConsolePreview();
            return;
        }

        // Agente de segurança: Ctrl+Alt+Del morto; menu TurboRama = Ctrl+End (todas as contas)
        // Tem de correr ANTES do single-instance do shell e em Admin também.
        if (HasArg(a, "--security-agent") || HasArg(a, "/security-agent"))
        {
            _logger.Info("Launcher", "Modo --security-agent (Ctrl+End → menu TurboRama).");
            Application.Run(new SecurityAgentApp(_logger));
            return;
        }
        ...
        if (IsAdministratorUser(user) || IsMachineOrServiceAccount(user))
        {
            _logger.Info("Launcher",
                "Conta administrativa/serviço — sem UI shell. Agente de segurança pode correr com --security-agent.");
            if (Environment.UserInteractive && !IsSessionZero())
            {
                TryShowInfoOnce(
                    ...
                    "Preview:\n" +
                    "  TurboRama.Launcher.exe --test-operator");
            }
        ...
        bool operatorConsole = config.EnableOperatorConsole;

        // Escudo CAD na sessão + agente Ctrl+End (substitui menu Windows)
        if (operatorConsole)
        {
            CadRuntimeShield.ApplyCurrentUser();
            _secureDesktop = new SecureDesktopWatcher();
            // Agente autónomo com message loop (funciona mesmo com shell kiosk / sem Explorer)
            try { SecurityRegistration.RegisterEverywhere(_logger); } catch { /* ignore */ }
            try { SecurityRegistration.EnsureAgentRunning(_logger); } catch { /* ignore */ }
            _logger.Info("Launcher", "CadRuntimeShield + SecurityAgent (Ctrl+End) + SecureDesktopWatcher.");
        }

        // Hook no processo shell (backup se o agente falhar)
        if (operatorConsole || _hookEnabled)
        {
            OperatorHotkeyService.Install(alsoBlockWinAndAltTab: _hookEnabled || operatorConsole);
            _logger.Info("Launcher",
                "OperatorHotkey shell " + (operatorConsole ? "Ctrl+End ON" : "OFF") +
                " | blockWinAltTab=" + (_hookEnabled || operatorConsole));
        }

        // Hook antigo separado só se quiser bloqueios sem operator (compat)
        if (_hookEnabled && !operatorConsole)
        {
            InstallKeyboardHook();
            _logger.Info("Launcher", "Keyboard hook ATIVO (config EnableLauncherKeyboardHook=true).");
        }
        else if (!_hookEnabled)
        {
            _logger.Info("Launcher", "Keyboard hook bloqueios Win/AltTab OFF (default).");
        }
        ...
        // PIN do painel (não grava em log)
        _operatorPin = ResolveOperatorPin(config);
        ...
        OperatorHotkeyService.Uninstall();
        ReleaseSingleInstance();
    }

    private static string ResolveOperatorPin(ProductConfiguration config)
    {
        if (!string.IsNullOrWhiteSpace(config.OperatorPin))
        {
            return config.OperatorPin.Trim();
        }
    ...
    /// <summary>
    /// Como o CAD: poll do atalho + retorno do desktop seguro Winlogon.
    /// </summary>
    private static void PumpOperatorHotkey()
    {
        if (_operatorUiBusy)
        {
            return;
        ...
        {
            _secureDesktop?.Poll();
            if (_secureDesktop != null && _secureDesktop.ConsumePendingOpen())
            {
                OperatorHotkeyService.RequestOpenFromSecureDesktop();
                _logger?.Info("Launcher", "Retorno do desktop seguro (Ctrl+Alt+Del) → painel TurboRama.");
            }
        }
        catch
        {
        ...
        if (!OperatorHotkeyService.ConsumeOpenRequest())
        {
            return;
        }
        ...
        {
            _logger?.Error("Launcher", "OperatorConsole: " + ex.Message, errorCode: "TR-OP");
        }
    }

    private static void ShowOperatorConsole()
    {
        if (_operatorUiBusy)
        {
            return;
        ...
        _operatorUiBusy = true;
        OperatorHotkeyService.MarkConsoleOpen(true);
        try
        {
            // Como o CAD: sobe por cima de tudo (incluindo o jogo)
            _logger?.Info("Launcher", "Painel segurança TurboRama aberto (equiv. Ctrl+Alt+Del / Alt+End).");
            using var form = new OperatorConsoleForm(_operatorPin);
            form.TopMost = true;
            form.ShowDialog();
            var action = form.ChosenAction;
            _logger?.Info("Launcher", "Painel segurança ação=" + action);

            if (action is OperatorConsoleForm.OperatorAction.PowerOffMachine
                or OperatorConsoleForm.OperatorAction.RebootMachine)
            {
                EnterPowerLock(action == OperatorConsoleForm.OperatorAction.RebootMachine ? "reboot" : "shutdown");
                ForceCloseFrontendUi();
                RunPowerOffSequence(reboot: action == OperatorConsoleForm.OperatorAction.RebootMachine);
            }
            else
            {
                OperatorConsoleForm.ExecuteAction(action);
            }
        }
        finally
        {
            OperatorHotkeyService.MarkConsoleOpen(false);
            _operatorUiBusy = false;
            // limpar pedidos extra acumulados durante o diálogo
            while (OperatorHotkeyService.ConsumeOpenRequest()) { /* drain */ }
            _secureDesktop?.ConsumePendingOpen();
        }
    }

    private static void RunOperatorConsolePreview()
    {
        try
        {
            ConfigurationStore.Load(out ProductConfiguration config);
            _operatorPin = ResolveOperatorPin(config);
            _logger?.Info("Launcher", "PREVIEW painel operador (Alt+End) — sem kiosk");
            using var form = new OperatorConsoleForm(_operatorPin);
            form.ShowDialog();
            _logger?.Info("Launcher", "PREVIEW operador ação=" + form.ChosenAction);
            // Preview: não executa reboot/shutdown reais
        }
        catch (Exception ex)
        {
            _logger?.Error("Launcher", "PREVIEW operator: " + ex.Message, errorCode: "TR-PREVIEW-OP");
            try
            {
                MessageBox.Show("Falha no preview do painel:\n" + ex.Message, "TurboRama",
