...
/// <summary>
/// Loading TurboRama — fundo ondas estilo PlayStation 3 (XMB / água).
/// Painel central limpo. Preview: --test-loading
/// </summary>
internal sealed class LoadingScreenForm : Form
{
    ...
    private float _t;
    private string _status = "POWER ON";
    private string _detail = "TURBORAMA";
    ...
    public void ShowBrandHold(int minMs, Action<int, string>? onTick = null)
    {
        ...
        // ~30 fps: tempo real → barra avança a cada frame (nunca fica presa em 00)
        ...
        while (!_holdFinished && gate.ElapsedMilliseconds < safety)
        {
            ...
            // Animação das ondas no mesmo ritmo do frame
            _t += 0.033f;
    ...
    {
        if (t < 0.2f) { _status = "POWER ON"; _detail = "Sistema a iniciar..."; }
        else if (t < 0.4f) { _status = "LOADING"; _detail = "A carregar TURBORAMA..."; }
        else if (t < 0.65f) { _status = "LOADING"; _detail = "Preparando sessão arcade..."; }
        else if (t < 0.9f) { _status = "READY"; _detail = "Quase pronto..."; }
        else { _status = "START"; _detail = "A abrir o jogo..."; }
    }
    ...
    private void DrawContent(Graphics g, Rectangle box, bool blink)
    {
        ...
        {
            g.DrawString("TURBORAMA ARCADE SYSTEM", f, b, x, box.Bottom - 36);
        }
    }
